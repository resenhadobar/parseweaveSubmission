...
export function resolvePlayerCollision(
  ...
  resolved.x = THREE.MathUtils.clamp(
    resolved.x,
    worldBounds.minX + radius,
    worldBounds.maxX - radius
  )
  resolved.z = THREE.MathUtils.clamp(
    resolved.z,
    worldBounds.minZ + radius,
    worldBounds.maxZ - radius
  )
  return resolved
}
...
export function collides(x: number, z: number, radius: number): boolean {
  if (
    x < worldBounds.minX + radius ||
    x > worldBounds.maxX - radius ||
    z < worldBounds.minZ + radius ||
    z > worldBounds.maxZ - radius
  )
    return true
  return (
    buildingColliders.some((bounds) => circleHitsBounds(x, z, radius, bounds)) ||
    propColliders.some((circle) => Math.hypot(x - circle.x, z - circle.z) < radius + circle.radius)
  )
}
