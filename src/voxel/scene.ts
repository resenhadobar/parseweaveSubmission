...
export function createBeachBlockScene(): THREE.Group {
  ...
  console.info(
    '[VoxelBeach] Applied Malibu roofs, fuller lawn planting, and animated traffic-ready scene props'
  )
  return world
}
...
function addBeachTowels(world: THREE.Group): void {
  ...
  for (let i = 0; i < 18; i += 1) {
    ...
    const z = -25 + (i % 4) * 3
    addBlock(world, {
      color: towelColors[i % towelColors.length],
      position: [x, 0.12, z],
      scale: [2.4, 0.1, 1.25],
    })
    addBlock(world, {
      color: 'white',
      position: [x - 0.65, 0.22, z + 0.22],
      scale: [0.55, 0.1, 0.3],
    })
  }
}
...
function addVolleyballNet(world: THREE.Group): void {
  ...
  for (let i = 0; i < 8; i += 1)
    addBlock(world, {
      color: 'white',
      position: [18 + i * 0.9, 1.08, -20],
      scale: [0.08, 1.45, 0.08],
    })
}
...
function createRoadDetails(world: THREE.Group): void {
  for (const road of roads) {
    if (road.direction === 'horizontal') {
      ...
      for (let x = road.minX + 4; x <= road.maxX - 4; x += 7)
        addBlock(world, { color: 'yellow', position: [x, 0.18, z], scale: [1.55, 0.05, 0.11] })
      for (let x = road.minX + 8; x <= road.maxX - 8; x += 12)
        addStreetLight(world, x, road.minZ - 1.55)
    } else {
      const x = (road.minX + road.maxX) / 2
      for (let z = road.minZ + 4; z <= road.maxZ - 4; z += 7)
        addBlock(world, { color: 'yellow', position: [x, 0.18, z], scale: [0.11, 0.05, 1.55] })
      for (let z = road.minZ + 8; z <= road.maxZ - 8; z += 12)
        addStreetLight(world, road.maxX + 1.55, z)
    }
  }
  ...
  for (const horizontal of roads.filter((road) => road.direction === 'horizontal')) {
    for (const vertical of roads.filter((road) => road.direction === 'vertical')) {
      addCrosswalk(
        world,
        (vertical.minX + vertical.maxX) / 2,
        (horizontal.minZ + horizontal.maxZ) / 2,
        'horizontal'
      )
      addCrosswalk(
        world,
...
function createLargeBuilding(lot: Lot): THREE.Group {
  ...
  addBlock(group, {
    ...
    scale: [lot.width, height, lot.depth],
  })
  addBlock(group, {
    color: lot.body,
    position: [-lot.width / 2 + 1.05, Math.min(3.4, height / 2), -0.55],
    scale: [1.7, Math.min(6, height * 0.62), lot.depth + 0.2],
  })
  addBlock(group, {
    color: 'wood',
    position: [0, height + 0.08, 0],
    scale: [lot.width + 0.55, 0.14, lot.depth + 0.45],
  })
  addBlock(group, {
    color: lot.roof,
    position: [0, height + 0.22, 0],
    scale: [lot.width + 0.28, isHotel ? 0.18 : 0.24, lot.depth + 0.24],
  })
  addBlock(group, {
    color: lot.roof,
    position: [0, height + 0.44, 0],
    scale: [lot.width - 1.05, 0.16, lot.depth - 0.68],
  })
  addBlock(group, {
    color: 'white',
    position: [0, height + 0.52, -lot.depth / 2 + 0.48],
    scale: [lot.width - 1.1, 0.1, 0.12],
  })
  ...
  if (isHotel)
    addBlock(group, { color: 'metal', position: [0, height + 1.05, 0], scale: [0.28, 1.6, 0.28] })
  return group
}
...
function addStreetFacingDoor(group: THREE.Group, lot: Lot): void {
  ...
  addBlock(group, {
    color: lot.roof,
    position: [0, 2.18, frontZ - Math.sign(frontZ) * 0.18],
    scale: [2.1, 0.28, 1.05],
  })
  addBlock(group, {
    color: 'metal',
    position: [0, 0.26, frontZ + (frontZ > 0 ? 0.03 : -0.03)],
    scale: [1.25, 0.16, 0.18],
  })
}

function addLargeBuildingWindows(
  group: THREE.Group,
  lot: Lot,
  floorHeight: number,
  isHotel: boolean
  ...
  for (let floor = 0; floor < lot.stories; floor += 1) {
    ...
    for (let col = 0; col < cols; col += 1) {
      ...
      addBlock(group, {
        color: isHotel ? 'darkGlass' : 'glass',
        position: [x, y, -lot.depth / 2 - 0.05],
        scale: [size, 0.58, 0.08],
      })
      addBlock(group, {
        color: isHotel ? 'darkGlass' : 'glass',
        position: [x, y, lot.depth / 2 + 0.05],
...
function addStackedBalconies(group: THREE.Group, lot: Lot, floorHeight: number): void {
  for (let floor = 1; floor < Math.min(lot.stories, 6); floor += 1) {
    ...
    for (const x of [-lot.width / 2 + 1.4, lot.width / 2 - 1.4]) {
      addBlock(group, {
        color: 'wood',
        position: [x, y, -lot.depth / 2 - 0.32],
        scale: [1.38, 0.16, 0.52],
      })
      addBlock(group, {
        color: 'white',
        position: [x, y + 0.32, -lot.depth / 2 - 0.58],
        scale: [1.38, 0.12, 0.1],
      })
    }
  }
...
function addRooftopPersonality(
  group: THREE.Group,
  lot: Lot,
  height: number,
  isHotel: boolean
  ...
  addBlock(group, {
    color: 'white',
    position: [-lot.width / 2 + 1.05, height + 0.58, 0],
    scale: [0.12, 0.36, lot.depth - 1.05],
  })
  addBlock(group, {
    color: 'white',
    position: [lot.width / 2 - 1.05, height + 0.58, 0],
  ...
  addBlock(group, {
    color: 'metal',
    position: [lot.width / 2 - 1.5, height + 0.48, lot.depth / 2 - 1.15],
    scale: [0.95, 0.2, 0.62],
  })
  addBlock(group, {
    color: isHotel ? 'glass' : 'teal',
    position: [-lot.width / 2 + 1.55, height + 0.48, lot.depth / 2 - 1.1],
    scale: [1.05, 0.14, 0.62],
  })
  if (isHotel)
    addBlock(group, {
      color: lot.roof,
      position: [0, height + 0.68, -lot.depth / 2 + 1.05],
      scale: [2.2, 0.14, 0.78],
    })
}
...
function addCrosswalk(
  ...
  if (direction === 'horizontal') {
    for (const dz of [-offset, offset])
      addBlock(world, { color: 'white', position: [x, 0.2, z + dz], scale: [3.7, 0.05, 0.16] })
  } else {
    for (const dx of [-offset, offset])
      addBlock(world, { color: 'white', position: [x + dx, 0.2, z], scale: [0.16, 0.05, 3.7] })
  }
}
...
function addTunnelPortal(
  ...
  addBlock(world, {
    color: 'black',
    position: [x, 1.05, z],
    scale: horizontal ? [0.28, 2.1, 3.55] : [3.55, 2.1, 0.28],
  })
  addBlock(world, {
    color: 'rock',
    position: [x, 2.35, z],
    scale: horizontal ? [0.45, 0.55, 4.25] : [4.25, 0.55, 0.45],
  })
  addBlock(world, {
    color: 'rock',
    position: horizontal ? [x, 1.05, z - 1.95] : [x - 1.95, 1.05, z],
