...
export function createBeachHouse(options: HouseOptions = {}): THREE.Group {
  ...
  addBlock(group, {
    color: trim,
    position: [0, bodyHeight - 0.12, -depth / 2 - 0.05],
    scale: [width + 0.25, 0.18, 0.12],
  })
  addBlock(group, {
    color: trim,
    position: [0, 0.35, -depth / 2 - 0.06],
    scale: [width + 0.2, 0.22, 0.12],
  })
  ...
  if (!options.shop) {
    ...
    addBlock(group, {
      color: body,
      position: [wingSide * (width / 2 - 0.9), 1.15, 0.75],
      scale: [2.3, 2.3, 3.2],
    })
...
function addMalibuRoof(
  ...
  addBlock(group, {
    color: 'wood',
    position: [0, y + 0.08, 0],
    scale: [width + 0.78, 0.16, depth + 0.62],
  })
  addBlock(group, {
    color: roof,
    position: [0, y + 0.24, 0],
    scale: [width + 0.52, 0.22, depth + 0.42],
  })
  addBlock(group, {
    color: roof,
    position: [0, y + 0.48, 0],
    scale: [width - 0.75, 0.2, depth - 0.25],
  })
  addBlock(group, { color: roof, position: [0, y + 0.7, 0], scale: [0.72, 0.2, depth - 0.5] })
  addBlock(group, {
    color: trim,
    position: [0, y + 0.36, -depth / 2 - 0.08],
    scale: [width + 0.58, 0.12, 0.12],
  })
  addBlock(group, {
    color: trim,
    position: [0, y + 0.36, depth / 2 + 0.08],
...
function addPorch(
  ...
  for (const x of [porchX - 1.25, porchX + 1.25])
    addBlock(group, { color: trim, position: [x, 1.28, frontZ - 0.36], scale: [0.16, 1.85, 0.16] })
  addBlock(group, {
    color: 'wood',
    position: [porchX, 0.18, frontZ - 0.82],
    scale: [2.05, 0.2, 0.32],
  })
  addBlock(group, {
    color: 'darkGlass',
    position: [porchX, 1.18, -depth / 2 - 0.08],
    scale: [0.82, 1.58, 0.1],
  })
}
...
function addBalcony(
  ...
  addBlock(group, {
    color: roof,
    position: [balconyX, 3.28, frontZ - 0.35],
    scale: [2.45, 0.18, 0.14],
  })
  for (let i = -1; i <= 1; i += 1)
    addBlock(group, {
      color: trim,
      position: [balconyX + i * 0.72, 3.48, frontZ - 0.35],
      scale: [0.1, 0.5, 0.1],
    })
  addBlock(group, {
    color: 'glass',
    position: [balconyX, 3.82, -depth / 2 - 0.08],
    scale: [0.9, 0.62, 0.08],
  })
  addBlock(group, {
    color: trim,
    position: [balconyX, 4.24, -depth / 2 - 0.09],
    scale: [1.1, 0.14, 0.08],
  })
}
...
function addShopfront(
  ...
  addBlock(group, {
    ...
    scale: [width + 0.38, 0.34, depth + 0.35],
  })
...
function addHouseWindows(
  ...
  for (let floor = 0; floor < stories; floor += 1) {
    ...
    if (pattern === 'wide') {
      ...
      addBlock(group, {
        color: 'glass',
        position: [2.25, y + 0.1, frontZ],
        scale: [1.15, 0.54, 0.08],
      })
    } else {
      for (const x of [-width / 2 + 1.2 + rowOffset, width / 2 - 1.2 - rowOffset])
        addBlock(group, { color: 'glass', position: [x, y, frontZ], scale: [0.72, 0.58, 0.08] })
    }

    for (const z of [-depth / 2 + 1.45, depth / 2 - 1.45]) {
      addBlock(group, {
        color: 'glass',
        position: [-width / 2 - 0.06, y + 0.05, z],
        scale: [0.08, 0.5, 0.58],
      })
      addBlock(group, {
        color: 'glass',
...
function addPlanters(group: THREE.Group, width: number, depth: number, variant: number): void {
  ...
  for (const x of [-width / 2 + 0.8, width / 2 - 0.8]) {
    ...
    addBlock(group, {
      color: variant % 2 === 0 ? 'pink' : 'yellow',
      position: [x - 0.18, 0.72, frontZ],
      scale: [0.18, 0.2, 0.18],
    })
...
function addRoofTerrace(
  ...
  addBlock(group, {
    color: trim,
    position: [0, y + 0.28, -depth / 2 + 0.45],
    scale: [width - 1.15, 0.28, 0.16],
  })
  addBlock(group, {
    color: trim,
    position: [-width / 2 + 0.45, y + 0.28, 0],
    scale: [0.16, 0.28, depth - 1.2],
  })
  addBlock(group, {
    color: trim,
    position: [width / 2 - 0.45, y + 0.28, 0],
  ...
  addBlock(group, {
    color: accent,
    position: [width / 2 - 1.35, y + 0.22, depth / 2 - 1.15],
    scale: [1.2, 0.22, 0.95],
  })
  if (shop)
    addBlock(group, {
      color: 'glass',
      position: [-width / 2 + 1.35, y + 0.18, depth / 2 - 1.2],
      scale: [1.15, 0.14, 0.75],
    })
}
...
function addSurfboardRack(group: THREE.Group, x: number, z: number, variant: number): void {
  ...
  for (let i = 0; i < 3; i += 1) {
    addBlock(group, {
      ...
      position: [x, 0.82 + i * 0.06, z - i * 0.22],
      scale: [0.22, 1.55, 0.12],
    })
  }
...
export function createPalmTree(variant = 0): THREE.Group {
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    addBlock(group, {
      color: 'trunk',
      position: [trunkX, 0.48 + (i * height) / 5, 0],
      scale: [0.38, 0.82, 0.38],
    })
  }
  ...
  fronds.forEach((angle, index) => {
    ...
    for (let segment = 0; segment < 3; segment += 1) {
      ...
      addBlock(group, {
        ...
        scale: [
          Math.abs(direction.x) > 0.5 ? 0.92 : 0.58,
          0.2,
          Math.abs(direction.z) > 0.5 ? 0.92 : 0.58,
        ],
      })
    }
  })
  addBlock(group, {
    color: 'orange',
    position: [crownX + 0.24, crownY - 0.28, 0.18],
    scale: [0.22, 0.24, 0.22],
  })
  addBlock(group, {
    color: 'orange',
    position: [crownX - 0.2, crownY - 0.22, -0.12],
    scale: [0.2, 0.22, 0.2],
  })
  return group
}
...
export function createTropicalTree(variant = 0): THREE.Group {
  ...
  addBlock(group, {
    color: 'palm',
    position: [variant % 2 === 0 ? 0.9 : -0.9, 3.48, 0.2],
    scale: [1.9, 1.05, 1.75],
  })
...
function addCharacterLimb(
  ...
  addBlock(limb, {
    color: endColor,
    position: [0, -scale[1] - 0.06, -0.02],
    scale: [scale[0] + 0.06, 0.12, scale[2] + 0.08],
  })
  parent.add(limb)
}
...
export function createDeliveryBike(): THREE.Group {
  ...
  for (const x of [-0.34, 0.34])
    for (const z of [-0.58, 0.58])
      addBlock(group, { color: 'black', position: [x, 0.06, z], scale: [0.18, 0.18, 0.18] })
...
export function createBeachUmbrella(): THREE.Group {
  ...
  for (let i = 0; i < 8; i += 1) {
    ...
    addBlock(group, {
      ...
      position: [Math.cos(angle) * 0.72, 1.75, Math.sin(angle) * 0.72],
      scale: [0.95, 0.2, 0.42],
    })
  }
...
export function createLifeguardTower(): THREE.Group {
  ...
  for (const x of [-1.1, 1.1])
    for (const z of [-0.85, 0.85])
      addBlock(group, { color: 'wood', position: [x, 0.55, z], scale: [0.22, 1.1, 0.22] })
