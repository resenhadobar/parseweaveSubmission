...
describe('voxel neighborhood layout', () => {
  ...
  it('keeps lots from overlapping each other', () => {
    for (let i = 0; i < lots.length; i += 1) {
      for (let j = i + 1; j < lots.length; j += 1) {
        expect(
          boxesOverlap(lotBounds(lots[i]), lotBounds(lots[j])),
          `${lots[i].id} overlaps ${lots[j].id}`
        ).toBe(false)
      }
    }
  })
  ...
  it('keeps sidewalks adjacent to roads but never inside road lanes', () => {
    for (let x = worldBounds.minX; x <= worldBounds.maxX; x += 1) {
      for (let z = worldBounds.minZ; z <= worldBounds.maxZ; z += 1) {
        expect(isRoad(x, z) && isSidewalk(x, z), `sidewalk bleeds into road at ${x},${z}`).toBe(
          false
        )
      }
    }
  ...
  it('keeps all vehicles on their assigned road and facing lane direction', () => {
    for (const vehicle of vehicles) {
      ...
      expect(
        containsPoint(road!, vehicle.x, vehicle.z),
        `vehicle ${vehicle.color} is off ${vehicle.roadId}`
      ).toBe(true)

      if (road!.direction === 'horizontal') {
        const centerZ = (road!.minZ + road!.maxZ) / 2
        const expected = vehicle.z < centerZ ? 0 : Math.PI
        expect(vehicle.rotation, `horizontal car wrong way at ${vehicle.x},${vehicle.z}`).toBe(
          expected
        )
      } else {
        const centerX = (road!.minX + road!.maxX) / 2
        const expected = vehicle.x < centerX ? Math.PI / 2 : -Math.PI / 2
        expect(vehicle.rotation, `vertical car wrong way at ${vehicle.x},${vehicle.z}`).toBe(
          expected
        )
      }
    }
  ...
  it('uses coherent car, person, and building scale ratios', () => {
    ...
    for (const lot of lots) {
      ...
      expect(floorHeight, `${lot.id} story cannot fit a person`).toBeGreaterThan(
        personHeight * 1.35
      )
      expect(
        lot.stories * floorHeight,
        `${lot.id} height too close to person height`
      ).toBeGreaterThan(personHeight * 2.5)
    }
  })
  ...
  it('forms a taller horseshoe mountain range into beach and water edges', () => {
    ...
    expect(
      sampleMountainHeight(worldBounds.minX - 4, worldBounds.beachEndZ - 12)
    ).toBeGreaterThanOrEqual(8)
    expect(
      sampleMountainHeight(worldBounds.maxX + 4, worldBounds.beachEndZ - 12)
    ).toBeGreaterThanOrEqual(8)
  })
